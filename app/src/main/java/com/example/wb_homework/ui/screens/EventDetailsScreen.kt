...
internal fun EventDetailsScreen(
    ...
    when (val currentState = screenState.value) {
        is EventDetailsScreenState.EventDetails -> {
            EventDetails(event = currentState.event, viewModel) {
...
private fun EventDetails(
    ...
    val goingToEvent = viewModel.checkEventInDB().collectAsStateWithLifecycle()
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                title = {
                    Subheading1(text = event.eventName + event.id.toString())
                },
                navigationIcon = {
        ...
        Column(
            ...
            if (goingToEvent.value) {
                SecondaryInitialButton(
                    ...
                    viewModel.deleteMyEvent(event.id)
